...
export const PaidLicenseManagement: FC<Props> = (props: Props) => {
  const { t } = useTranslation();

  const TABLE_HEADER = [t('field.numeric'), t('field.featureType'), t('field.activationDate'), 'API Key', t('field.used')];
  ...
  const getUserLicense = (page: number) => {
    setIsLoading(true);
    getLicense({ page, page_size: pageSize, license_type: 1 })
      .then((res) => {
        setIsLoading(false);
