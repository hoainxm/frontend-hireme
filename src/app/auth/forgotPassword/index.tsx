...
export const ForgotPassword = (): ReactElement => {
  ...
  const onValid: SubmitHandler<ForgotPasswordFormInputs> = (data, event) => {
    setIsLoading(true)
    generateTokenResetPassword(data).then(() => {
      setErrorMessage("")
      setIsLoading(false)
  ...
  return (
    <AuthFormLayout
      ...
      subTitle="auth.forgotPass.subTitle"
      backTo={PageURL.LOGIN}
    >
      <Form
      ...
      >
        <Form.Group className={style.inputGroup}>
          ...
          {errors.email?.type === "required" && (
            <CInputHint>{t(`${errors.email.message}`)}</CInputHint>
          )}
          {errors.email?.type === "pattern" && (
            <CInputHint>{t("field.error.email")}</CInputHint>
          )}
          {errorMessage && (
            <CInputHint>{t(errorMessage)}</CInputHint>
          )}
        </Form.Group>
        <CButton
          type="submit"
          label={checkCountDown()}
