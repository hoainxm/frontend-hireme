...
export const ResetPassword = (props: Props): ReactElement => {
  ...
  const { errors, handleSubmit, getValues, register, watch } =
  ...
  const validateCfmPass = (data: string): boolean => {
    return data === getValues("new_password")
  };
  ...
  const checkShowPasswordRule = (): boolean => {
    ...
    return isPasswordFocused || errors.new_password?.type === "pattern"
  }
    ...
    return (
      ...
      >
        <div className={style.resetSuccessful}>
          ...
          <CButton
            label={t("btn.backToLogin")}
            className={`${style.btn} mb-0`}
      ...
      >
        <Form.Group className={style.inputGroup} ref={refPassword}>
          ...
          {errors.new_password?.type === "required" && (
            <CInputHint>{t(`${errors.new_password.message}`)}</CInputHint>
          )}
        </Form.Group>
        <FormatPasswordRule
          value={watch("new_password") || ""}
          target={refPassword} 
          isFocus={checkShowPasswordRule()} 
        />
        <Form.Group className={style.inputGroup}>
          <Form.Label>
            {t("field.newCfmPassword")}
          </Form.Label>
          <CInput
            id="confirm_password"
            autoComplete="off"
          ...
          {errors.confirm_password?.type === "required" && (
            <CInputHint>{t(`${errors.confirm_password.message}`)}</CInputHint>
          )}
          {errors.confirm_password?.type === "validate" && (
            <CInputHint>{t("field.error.confirm_password")}</CInputHint>
          )}
        </Form.Group>
        <CButton
          type="submit"
